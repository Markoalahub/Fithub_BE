...
import markoala.fithub.demo.project.dto.ProjectCreateResponse;
import markoala.fithub.demo.project.dto.ProjectInviteRequest;
import markoala.fithub.demo.project.exception.DuplicateProjectException;
...
public class ProjectControllerTest {
    ...
    void inviteUserToProject_Success() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/projects/1/invite")
                        .header("Authorization", "Bearer token")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isOk());
    ...
    void getProjectPipelines_Forbidden() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/projects/1/pipelines")
                        .header("Authorization", "Bearer token"))
                .andExpect(status().isForbidden());
    }
}
